...
class _c_random_drifter:
    ...
    def drift(self):
        ...
        x, y = self._init_pos()
        phase = self.calculate_phase(x,y)
    ...
    def _traverse_boundary(self,phase,x,y):
        ...
        while True:
            ...
            for ii in range(num_neighbors):
                ...
                new_phase = self.calculate_phase(new_x,new_y)
    ...
    def _check_neighbors(self,x,y):
        ...
        for ii in range(num_neighbors):
            ...
            if self.phase_diagram.phases[_x,_y] < 0:
                ...
                _phase = self.calculate_phase(_x,_y)
    ...
    def calculate_phase(self,x_ind,y_ind):
        ...
        phase = self.phase_diagram.calculate_phase(x,y)
...
class _c_new_random_drifter:
    ...
    def calculate_phase(self,x,y):
        ...
        x_coord = self.phase_diagram.x_coords[x]
        y_coord = self.phase_diagram.y_coords[y]

        # this expects x and y to be in the range [0,1]
        phase = self.phase_diagram.calculate_phase(x_coord,y_coord)
    ...
    def drift(self):
        ...
        """
        run the drifting algorithm. start drifting and keep taking steps until we hit a phase 
        boundary or go out of bounds. if out of bounds, stop drifting. if we hit a phase
        boundary, we switch over to algorithm for traversing the phase boundary.
        """
...
class c_phase_diagram:
    ...
    def __init__(self,calculate_phase,num_x,num_y,phase_diagram=None):

        """
        calculate_phase is call back that takes x and y coords and args and returns phase. expects
        ...
        to restart a phase diagram calculation, pass in a phase_diagram that has the
            phases already calculated. it should have the shape [num_x, num_y]
        """
    ...
    def _generate_coords(self):
        ...
        # coordinates are floats that span from 0 to 1 
        self.x_coords = np.linspace(0,1,self.num_x)
        self.y_coords = np.linspace(0,1,self.num_y)

        _x_grid, _y_grid = np.meshgrid(self.x_coords,self.y_coords,indexing='ij')
        self.coords = np.c_[_x_grid.flatten(), _y_grid.flatten()]
        self.grid_shape = _x_grid.shape

        # indices are integers that span from 0 to num_x, num_y, etc
        self.x_inds = np.arange(0,self.num_x)
        self.y_inds = np.arange(0,self.num_y)

        _x_grid, _y_grid = np.meshgrid(self.x_inds,self.y_inds,indexing='ij')
        self.inds = np.c_[_x_grid.flatten(),_y_grid.flatten()]
    ...
    def random_drifters(self,num_drifters=1):
        ...
        for ii in range(self.num_drifters):
            ...
            _drifter = _c_new_random_drifter(self,ii)
            _drifter.drift()
...
if __name__ == '__main__':
    ...
    calculate_phase = example.calculate_phase

    # phase_checker = c_phase_checker()
    # calculate_phase = phase_checker.calculate_phase
    
    phase_diagram = c_phase_diagram(calculate_phase,num_x,num_y)
    phase_diagram.random_drifters(20)
