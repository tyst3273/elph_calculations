...
fig, ax = plt.subplots(2,1,figsize=(4.5,6),gridspec_kw={'hspace':0.05,'wspace':0.1,'height_ratios':[0.75,1]})
...
for ii in range(num_modes):
    ...
    # ax[0].errorbar(qpts,freqs[:,ii],x,marker='o',ms=0,c='m',elinewidth=1,alpha=0.25)
    ...
    # ax[1].errorbar(qpts,freqs[:,ii],x,marker='o',ms=0,c='m',elinewidth=1,alpha=0.25)
    ...
    # hi = freqs[:,ii]+g
    # lo = freqs[:,ii]-g
    # ax[1][.fill_between(qpts,lo,hi,color='b',alpha=0.5)
...
plt.savefig(f'matrix_elements_2.png',dpi=300,bbox_inches='tight')
plt.show()
# plt.close()
